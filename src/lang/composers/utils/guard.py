...
from .strategies import (
    ...
    TransformPattern,
    StrategyType,
    serialise_application_strategy
)
...
class GuardRule:
    ...
    Attributes:
        ...
        condition: Function that returns True if strategy should be BLOCKED
                   Signature: (fn_name, arg_pos, strategy_type, other_strategies) -> bool
    """
    name: str
    fn_names: Optional[set[str]]
    arg_positions: Optional[set[int]]
    condition: Callable[[str, int, StrategyType, list[Optional[StrategyType]]], bool]
    ...
    def should_block(
        ...
        other_strategies: list[Optional[StrategyType]]
    ) -> bool:
...
class StrategyGuard:
    ...
    def _register_default_rules(self):
        ...
        self.add_rule(GuardRule(
            ...
            fn_names={'if'},
            arg_positions={0},
            condition=lambda fn, pos, strat, others: strat == StrategyType.LITERAL
        ))
        ...
        def binary_needs_nonliteral(fn: str, pos: int, strat: str, others: list[Optional[str]]) -> bool:
            if strat != StrategyType.LITERAL:
                return False
            # Block literal if ALL other args are also literals
            for i, other in enumerate(others):
                if i != pos and other is not None and other != StrategyType.LITERAL:
            ...
            if pos == len(others) - 1 or all(o is not None for o in others):
                return all(o == StrategyType.LITERAL for o in others if o is not None)
            return False
        ...
        self.add_rule(GuardRule(
            ...
            condition=lambda fn, pos, strat, others: strat == StrategyType.IDENTITY
        ))
    ...
    def should_block_strategy(
        ...
        Args:
            ...
            strategy_type: Type of strategy (StrategyType)
    ...
    def should_block_with_context(
        ...
        Args:
            ...
            strategy_type: StrategyType to check
    ...
    def _strategy_to_type(self, strategy: Strategy) -> str:
        ...
        if strategy.name == StrategyType.APPLY:
            return serialise_application_strategy(strategy)
        else:
    ...
    def guard_lambda_body(
        ...
        if body_strategy == StrategyType.VARIABLE and body_references_only_params:
            return True
        return False
